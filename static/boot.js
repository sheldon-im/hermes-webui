...
(async()=>{
  ...
  try{
    ...
    // Settings load failed: keep the persisted preference (eager default already
    // read it from the localStorage mirror) instead of clobbering it with 'queue'
    // so a saved 'steer'/'interrupt' still applies when the server is unreachable (#5167).
    window._busyInputMode=_readPersistedBusyInputMode();
